...
class AttachmentAPIViewsTestCase(APITestCase):
    """
    Тестирование Views API Attachment
    """
    ...
    def tearDown(self):
        ...
        self.attachment_1.delete()

    def test_attachment_upload_new(self):
        """
        path: projects/attachment/upload/
        Загрузка нового файла к проекту
        Только для модераторов и суперов и владельцев проекта
        """

        client = APIClient()
        url = reverse('projects:project_attachment_upload')

        file_mock = mock.MagicMock(spec=File, name='FileMock')
        file_mock.name = 'test1.jpg'

        data = {
            "uploaded_file": file_mock
        }
        ...
        settings.SAVED_PICTURE_MAX_FILE_SIZE_MB = 30 / 1024 / 1024

        client.credentials(HTTP_AUTHORIZATION='JWT ' + self.user_1_token)
        resp = client.post(url, data, format='multipart')
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)

        settings.SAVED_PICTURE_MAX_FILE_SIZE_MB = 12
    ...
    def test_attachment_update_new(self):
        """
        path: projects/link/update/<uuid:pk>/
        Тестируем удаление и изменение файлов проекта
        """

        client = APIClient()
        url = reverse('projects:project_attachment_update_delete',
                      kwargs={'pk': self.attachment_1.id})
        ...
        self.assertEqual(self.attachment_1.is_feature, False)

        resp = client.put(url, data, format='multipart')
        self.assertEqual(resp.status_code, status.HTTP_401_UNAUTHORIZED)
        ...
        self.attachment_1.refresh_from_db()
        self.assertEqual(self.attachment_1.is_feature, True)
